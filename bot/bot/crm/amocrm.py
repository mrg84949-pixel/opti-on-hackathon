...
DEFAULT_SLOTS_PATH = "/api/v4/appointments/slots"
DEFAULT_BOOKING_PATH = "/api/v4/appointments"
DEFAULT_STAFF_PATH = "/api/v4/appointments/doctors"
DEFAULT_LIST_APPOINTMENTS_PATH = "/api/v4/appointments"
DEFAULT_CANCELLED_STATUS_VALUE = "cancelled"


def _resolve_path(config: dict[str, Any], key: str, default: str) -> str:
...
class AmoCRMProvider:
    """
    MVP-адаптер amoCRM.
    Если креды не заданы, работает в demo-режиме (фиктивные слоты и ID записи).
    """
    ...
    async def book_appointment(
        ...
        if not crm_id:
            raise ValueError("amoCRM booking response does not contain appointment id")
        crm_id = str(crm_id)
...
def provider_from_env() -> AmoCRMProvider:
    if settings.tenant_config_strict:
        return provider_from_org(base_url=None, token=None)
...
def provider_from_org(
    ...
    *,
    ...
    return AmoCRMProvider(base_url=base_url, token=token, config=config)


def _parse_staff_row(row: dict[str, Any]) -> StaffMember:
    ...
    if isinstance(schedule, dict):
        work_start = work_start or schedule.get("start") or schedule.get("work_start")
        work_end = work_end or schedule.get("end") or schedule.get("work_end")
    if not work_start or not work_end:
        logger.warning("CRM staff %s missing schedule; using 08:00–20:00", staff_id or name)
    active_raw = row.get("active", row.get("is_active", True))
    return StaffMember(
        id=staff_id or name,
        name=name or staff_id,
        work_start=str(work_start or "08:00"),
        work_end=str(work_end or "20:00"),
        active=bool(active_raw),
    )


def _extract_staff_rows(payload: Any) -> list[dict[str, Any]]:
    if isinstance(payload, list):
        return [row for row in payload if isinstance(row, dict)]
    if not isinstance(payload, dict):
        return []
    for key in ("doctors", "staff", "items", "data"):
...
def _extract_appointment_rows(payload: Any) -> list[dict[str, Any]]:
    if isinstance(payload, list):
        return [row for row in payload if isinstance(row, dict)]
    if not isinstance(payload, dict):
        return []
...
def _parse_appointment_snapshot(row: dict[str, Any]) -> CrmAppointmentSnapshot | None:
    crm_id = row.get("id") or row.get("appointment_id")
    status = row.get("status")
    if not crm_id or status is None:
        return None
    return CrmAppointmentSnapshot(
        crm_appointment_id=str(crm_id),
